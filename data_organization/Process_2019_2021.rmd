...
years <- c(2019, 2020, 2021)%>%paste0("_cycle")

hydro_share_datasets <- map(years, function(year_cycle){
  
  
drift_corrected_files <- list.files(path = here("data/raw/sensor/manual_data_verification/", year_cycle,"/post_verification/"), pattern = "*.parquet", full.names = TRUE)

drift_corrected_data <- map_dfr(drift_corrected_files, 
                                ~read_parquet(.x)%>% 
                                  ross.wq.tools::add_column_if_not_exists(., "pre_post_source")%>%
                                  mutate(pre_post_source = as.character(pre_post_source)))%>%
...
non_drift_corrected_files <- list.files(path = here("data/raw/sensor/manual_data_verification/", year_cycle, "in_progress/verified_directory/"), pattern = "*.parquet", full.names = TRUE)
...
raw_dataset <- all_datasets%>%
  ...
  mutate(value_post_drift_corr = if_else(is.na(value_post_drift_corr), value_post_ver, value_post_drift_corr),
         ...
         modifications = case_when( is.na(value_post_ver) & !is.na(value_raw) ~ "manual data exclusion",
                                    value_post_ver != value_post_drift_corr ~ "drift correction",
                                    TRUE ~ NA), 
         #convert to UTC
...
```{r}
# 
final_dataset <- map(hydro_share_datasets, "final_dataset")%>%
  bind_rows()%>%
...
plots <- map(unique(final_dataset$site), function(site_name) {
  ggplot(final_dataset %>% filter(site == site_name), aes(x = DateTime_UTC, y = value_final, color = modifications)) +
    geom_point(aes(y = value_raw), alpha = 0.5) + # add raw data points for comparison
    geom_point() +
    facet_wrap(~parameter, scales = "free_y") +
...
#plot specific site and parameter
final_dataset%>%
  filter(site == "archery", parameter == "Specific Conductivity")%>%
  ggplot(aes(x = DateTime_UTC, y = value_final, color = modifications)) +
  geom_point(aes(y = value_raw), alpha = 0.5) + # add raw data points for comparison
  geom_point() +
  labs(title = "Archery - Turbidity",
       x = "Date",
       y = "Value",
       color = "QC Status")
```
...
for(year in c("2019", "2020", "2021")){
  ...
  final_dataset <- hydro_share_datasets[[year_cycle]][["final_dataset"]]
  raw_dataset <- hydro_share_datasets[[year_cycle]][["raw_dataset"]]
...
old_field_notes <- readxl::read_excel(here("data","raw","field_notes","sensor_field_notes.xlsx")) %>% 
  ...
  mutate(rdo_pre_clean = NA, 
         ...
         chla_post_cal = NA)%>%
  select(-sensors_cleaned, -wiper_working, -cal_report_collected)%>%
  filter(DT_round <= ymd("2021-12-31"))

formatted_notes <- old_field_notes%>%
  filter(!(site %in% c("springcreek", "boxcreek", "mountaincampus")))%>%
  rename(DateTime_UTC = DT_round) %>%
  select(-any_of(c("photos_downloaded","DT_join", "cal_report_collected", "sensor_swapped_notes", "end_dt", "last_site_visit", "date","sensor_deployed", "sensor_pulled")))%>%
...
#Only 2021 had field notes
  
  field_notes_file_name <- here( "data/collated/sensor/hydroshare_pub/2021/CSU_ROSS_2021_field_notes.csv")
  
  write_csv(formatted_notes, field_notes_file_name)
