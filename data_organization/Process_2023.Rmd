...
log_directory <- here("data","raw", "sensor",  "log_download", "2023")  #Change as needed
...
### Clear folders
If you are re-running this script, you may want to clear out the staging and flagged_temp folders to avoid duplicate data.
This is useful as we re-run the workflow throughout the field season for reporting and QA/QC purposes.

```{r}
# list.files(staging_directory, full.names = TRUE) %>%
#   walk(file.remove)
# list.files(flagged_temp_dir, full.names = TRUE) %>%
#   walk(file.remove)
# list.files(flagged_directory, full.names = TRUE) %>%
#   walk(file.remove)
...
Run this code chunk to launch the calibration correction tool. The app will open in a new window or tab in your default web browser.
```{r, echo=FALSE}
shiny::shinyAppDir(
...
## Prep data for manual verification shiny app
...
```{r}
  ...
  # We should expect to see a diurnal pattern in the temperature data, with peaks around ~ 15:00 (3pm) and troughs around ~ 5:00/7:00 (5-7am). If we don't see this pattern, it could be an indication that the timestamps are not correct or that there is some other issue with the data.
  # pre_processed_data%>%
...
This tool is located in the folder `shiny_ver_tool`. Here we are manually verifying the data, making any necessary corrections and then saving the output in the "verified_directory" folder for this year. The key columns added are:
- final_status: "PASS" if the user has verified the data as good, "FLAGGED" if the user has flagged the data for suspect but still useable data, "OMIT if the user has concluded that the data should not be used in analysis due to sensor error. 
- user_flag: more specific flag added by the user to indicate why they flagged/omitted the data (e.g. "suspect data", "sonde burial", "frozen water", "unsubmerged", "depth shift", "drift", "field visit", "other malfunction" etc). This is for posterity and to help inform auto-flagging performance. 
...
This tool is located in the script `drift_correction_tool`. Here we are applying the drift corrections identified in the manual verification tool to the data and then saving the output in the "post_verification" folder. 
...
A few sites have turbidity values that are mis calibrated and primarily need a small offset to correct. This was not corrected in calibration correction tool
...
raw_dataset <- all_datasets%>%
  ...
  mutate(
    #keep turb and fDOM from archery site from 9/2023 and 12/23
    ...
    #keep fDOM and Chl-a from riverbend and cottonwood sites from 9/2023 and 12/23
